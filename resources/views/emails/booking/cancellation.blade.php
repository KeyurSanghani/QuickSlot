...
@section('title', __('message.booking_cancellation_subject', ['service' => $service->name]))

@section('content')
    <!-- Main Content -->
    <h2 style="color: #2D3748; font-size: 24px; margin-bottom: 20px; text-align: center;">
        {{ __('message.booking_cancellation_subject', ['service' => $service->name]) }}
    </h2>

    <p style="color: #4A5568; font-size: 16px; margin-bottom: 20px;">
        {{ __('message.booking_cancelled_greeting', ['name' => $booking->client_name]) }}
    </p>

    <p style="color: #4A5568; font-size: 16px; margin-bottom: 20px;">
        {{ __('message.booking_cancelled_intro') }}
    </p>
    ...
    <div style="background-color: #F7FAFC; padding: 15px; border-radius: 5px; margin: 20px 0;">
        ...
        <p style="color: #4A5568; font-size: 14px; margin: 5px 0;">
            <strong>{{ __('message.cancelled_at') }}:</strong> {{ $booking->cancelled_at?->format(config('constant.DEFAULT_DATE_TIME_FORMAT')) }}
        </p>
        @if($cancellationReason)
        <p style="color: #4A5568; font-size: 14px; margin: 5px 0;">
            <strong>{{ __('message.cancellation_reason') }}:</strong> {{ $cancellationReason }}
        </p>
        @endif
    </div>
    ...
    <div style="margin: 20px 0; padding: 15px; border-left: 4px solid #667eea;">
        <p style="color: #4A5568; font-size: 14px; margin-bottom: 10px;">
            {{ __('message.booking_cancelled_footer') }}
        </p>
    </div>
    ...
    <div style="text-align: center; margin: 30px 0;">
        <a href="{{ config('app.url') }}" style="display: inline-block; padding: 12px 30px; background-color: #667eea; color: #ffffff; text-decoration: none; border-radius: 5px; font-size: 16px; font-weight: 600;">
            {{ __('message.book_another_appointment') }}
        </a>
    </div>
