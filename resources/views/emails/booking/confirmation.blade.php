...
@section('title', __('message.booking_confirmation_subject', ['service' => $service->name]))

@section('content')
    <!-- Main Content -->
    <h2 style="color: #2D3748; font-size: 24px; margin-bottom: 20px; text-align: center;">
        {{ __('message.booking_confirmation_subject', ['service' => $service->name]) }}
    </h2>

    <p style="color: #4A5568; font-size: 16px; margin-bottom: 20px;">
        {{ __('message.booking_confirmed_greeting', ['name' => $booking->client_name]) }}
    </p>

    <p style="color: #4A5568; font-size: 16px; margin-bottom: 20px;">
        {{ __('message.booking_confirmed_intro') }}
    </p>
    ...
    <div style="background-color: #F7FAFC; padding: 15px; border-radius: 5px; margin: 20px 0;">
        ...
        <p style="color: #4A5568; font-size: 14px; margin: 5px 0;">
            <strong>{{ __('validation.attributes.price') }}:</strong> {{ config('constant.CURRENCY_SYMBOL') }}{{ number_format($service->price, 2) }}
        </p>
        @if($booking->notes)
        <p style="color: #4A5568; font-size: 14px; margin: 5px 0;">
    ...
    <div style="margin: 20px 0; padding: 15px; border-left: 4px solid #667eea;">
        <p style="color: #4A5568; font-size: 14px; margin-bottom: 10px;">
            {{ __('message.booking_confirmed_footer') }}
        </p>
        <p style="color: #4A5568; font-size: 14px; margin: 5px 0;">
